# Ecommerce Buying Cycle Script

## Video Promise

By the end of this video, a Shopify store owner should understand the basic buying cycle and be able to look at their store as a sales journey, not just a collection of pages and products.

## Opening Hook

Preferred hook direction:

People do want to buy from small local brands. I genuinely believe that.

They just do not necessarily trust your store yet.

That sounds harsh, but online the customer cannot look you in the eye, ask a question over the counter, or see whether the shop feels real. Your Shopify store has to build that confidence for you.

And the ironic thing is that JB Hi-Fi and The Good Guys often feel more personable and trustworthy online than a local store does. Not because they are naturally more human, but because they spend millions making their websites feel safe, helpful, and reassuring.

So today we are going to look at the ecommerce buying cycle: the steps your store needs to take a customer through before they are comfortable buying from you.

See `hooks.md` for alternate opening tests.

## Story Arc

Use this as the spine for the full script:

- **Premise**: Most people want to support local brands, and small business owners assume that goodwill should help online.
- **But**: Local goodwill does not overcome a Shopify store that feels unclear, risky, sterile, or hard to buy from.
- **Therefore**: Use big-brand ecommerce patterns as a practical reference point, because big brands spend heavily to feel safe, helpful, and personable online.
- **But**: The buying cycle is not only about what is on the page; the wrong email or remarketing message at the wrong time can also kill the sale.
- **Therefore**: Store pages and messages need to match where the customer is: Find, Trust, Decide, Buy, or Come Back.
- **But**: Fixing one stage does not automatically fix the next. A customer might find the product but not trust the store, trust the store but still have objections, or decide to buy but hit friction.
- **Therefore**: Review the store stage by stage using real examples from The Good Guys, Koala, and Good Pair Days.
- **Resolution**: The viewer leaves with a practical checklist for spotting where their Shopify store is losing customers, plus a clear Fix-It Call CTA if they want help prioritising fixes.

## Structure

1. Local goodwill sounds like an advantage, but trust still has to be earned online.
2. Big brands can ironically feel more personable and trustworthy online, therefore we can learn from how they deliberately build reassurance.
3. A physical shop can sell through human interaction, therefore a Shopify store has to do that work through pages and messages.
4. The wrong message at the wrong time can kill a sale, therefore behaviour should guide email/SMS/remarketing.
5. The buying cycle gives us a simple map: Find, Trust, Decide, Buy, Come Back.
6. Each stage solves one problem, but exposes the next risk.
7. Real Australian stores show how those stages are handled in practice.
8. The viewer audits their own store stage by stage.
9. Fix-It Call CTA.

## Full Script

To be drafted after the brief/framework is approved.

Early note to include:

I am not saying your store needs to become The Good Guys or Koala. I work in and around large ecommerce projects and agencies, so I get to see how serious ecommerce teams think about these problems. The useful thing is translating that thinking into practical checks a smaller Shopify store can actually use.

If you market the wrong message at the wrong time, you can kill a sale. This is where Klaviyo, Shopify Email, Mailchimp, SMS, abandoned cart emails, browse abandonment, and post-purchase flows need to match behaviour.

An abandoned cart discount might be useful in some situations, but it is not automatically the right answer. If the customer left because they did not trust the delivery timing, did not understand sizing, were unsure about compatibility, or had not seen enough proof, a discount does not fix the real problem. The message has to match where they are in the buying cycle.

## Screen Recording Beats

- Show a simple diagram or notes page with the buying-cycle stages.
- Inspect The Good Guys for trust, safety, product discovery, and purchase confidence.
- Inspect Koala for objection handling, reassurance, and buying action.
- Inspect Good Pair Days for wine discovery, trust, product education, repeat purchase, and category-specific objections.
- Show a checklist-style summary at the end.

## On-Screen Callouts

- "Your store is a sales conversation."
- "Traffic is only the start."
- "Customers need orientation, trust, answers, confidence, and a clean next step."
- "Find the stage where people are dropping out."
- "Wrong message, wrong time, lost sale."
- "Do they need a discount, or do they need an answer?"

## CTA

If you want help looking at your Shopify store through this buying-cycle lens, book a Fix-It Call. We will focus on where customers are getting stuck, work through what is safe to fix live, and leave you with clear priorities either way.

## Clip Candidates

- "Your Shopify store is not a shelf. It is a sales conversation."
- "In a physical store, you can answer questions. Online, your product page has to do it."
- "If traffic is coming in but sales are not, the issue may be the journey between interest and confidence."
- "Most abandoned carts start before the cart."
- "Stop sending discounts when customers need answers."
